# Gets model predictions of images in a given directory
# Outputs predictions to predict.txt
...
if sys.version_info.minor <= 5:
    VERBOSE = False
    print("Use Python >= 3.5 for extra runtime information")

if VERBOSE:
    print(f"\nCurrent Tensorflow Version: {TF_VERSION}")

# try:
if int(TF_VERSION.split(".")[1]) > 1:
    # if using TF 2.3.1
    t = time()
    safetynet = tf.keras.models.load_model("./src/model/safetynet.tf")
    robustnet = tf.keras.models.load_model("./src/model/robustnet.tf")
    safetynet.trainable = False
    robustnet.trainable = False

    if VERBOSE:
        t2 = time() - t
        print(f"Models loaded successfully ({t2:.6f} s)")
    
# except:
else:
    if VERBOSE:
        print("Failed to load full models. Rebuilding with model weights")
        print("Upgrade Tensorflow version to >= 2.3.1 to load full models")
        t = time()
    ...
    if VERBOSE:
        t2 = time() - t
        print(f"Model download time: {t2:.6f} s")
        t = time()
    ...
    if VERBOSE:
        t2 = time() - t
        print(f"Models rebuilt with saved weights ({t2:.6f} s)")
        t = time()
...
if VERBOSE:
    t2 = time() - t
    print(f"{len(fs)} images loaded from {INPUT_PATH}/  ({t2:.6f} s)\n")
    end = len(fs) if (len(fs) < 12) else 12
    for i in range(end):
        print(f"  {fs[i]}")
    print("  ...\n  (etc.)\n")
    t = time()
...
if VERBOSE:
    t2 = time() - t
    print(f"Inference performed on {len(preds)} images ({t2:.6f} s)")
    t = time()
...
if VERBOSE:
    t2 = time() - t
    print(f"Predictions written to file ({t2:.6f} s)")
